...
<div class="panel panel-flat">
	<div class="panel-body" style="overflow: scroll;">
		...
		<form action="./boardCorrApproveStatus" method="POST">
					...
					<table class="table datatable-basic table-bordered">
						<thead>
							<tr>
								...
								<th>AreaNo</th>
								<th>Pipe(inch)</th>
								<th>Board Correction Amt</th>
								<th>Penalty Correction Amt</th>
								<th>Rebate Correction Amt</th>
								<th>Month Year</th>
								<th>Requested By</th>
						...
						<tbody>
							...
							<c:forEach var="app" items="${masterListApproval}">
								<tr>
									...
									<td>${app.area_no}</td>
									<td>${app.pipe_size}</td>
									<td>${app.board_adj_amount}</td>
									<td>${app.penalty_adj_amount}</td>
									<td>${app.rebate_adj_amount}</td>
...
$(document).ready(function() {
		...
		$('#arrearsPenaltyCorrectionScreen').show();
		$('#arrearsPenaltyCorrectionModule').addClass('active');

		var activeMod = "${activeModulesNew}";
		var module = "Arrears Correction Approval";
					...
					success : function(response) {
						...
						for (var i = 0; i < response.length; i++) {
							...
							if(data.connection_no == null ){ConnectionData+="<td></td>";}else{ ConnectionData+="<td>"+data.connection_no+"</td>"; }
					  		...
					  		if(data.board_adj_amount == null ){ConnectionData+="<td></td>";}else{ ConnectionData+="<td>"+data.board_adj_amount+"</td>";}
					  		if(data.bill_adj_amount == null ){ConnectionData+="<td></td>";}else{ ConnectionData+="<td>"+data.bill_adj_amount+"</td>";}
					  		if(data.rebate_adj_amount == null ){ConnectionData+="<td></td>";}else{ ConnectionData+="<td>"+data.rebate_adj_amount+"</td>";}
