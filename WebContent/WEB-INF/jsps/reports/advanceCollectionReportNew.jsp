	...
	<div class="panel panel-flat">
		<div class="panel-body">
			<fieldset class="content-group">
				<legend class="text-bold">Advance Collection Report</legend>
				<div class="row">
					<div class="col-md-3">
				...
				<div class="row">
					...
					<div class="form-group" style="padding-top: 27px;">
						<label>&nbsp;</label>
						<button type="button" class="btn bg-teal btn-ladda"
							onclick="return generateAdvanceReport()"
					...
					<div class="pdf-page">
						<kendo:grid name="categoryGrid" pageable="true"
							...
							<kendo:grid-columns>
								...
								<kendo:grid-column title="Opening Adv"
									field="opening_adv" width="120px" filterable="true"
									aggregates="sum"
									groupFooterTemplate="#: kendo.toString(sum, 'n2') #"
									footerTemplate="#:kendo.toString(opening_adv,'n2') #" />
								...
								<kendo:grid-column title="Bill Adjustment" field="adjustments"
									filterable="true" width="90px" aggregates="sum"
									groupFooterTemplate="#: kendo.toString(sum, 'n2') #"
									footerTemplate="#:kendo.toString(adjustments, 'n2','0.00') #" />
								
								
								
								<kendo:grid-column title="Adv Coll" field="current_adv_amt" width="87px"
									filterable="true" aggregates="sum"
									groupFooterTemplate="#: kendo.toString(sum, 'n2') #"
									footerTemplate="#:kendo.toString(current_adv_amt, 'n2','0.00') #" />
									
								<kendo:grid-column title="Adv Rebate" field="current_adv_rebate"
									filterable="true" width="100px" aggregates="sum"
									groupFooterTemplate="#: kendo.toString(sum, 'n2') #"
									footerTemplate="#:kendo.toString(current_adv_rebate, 'n2','0.00') #" />
									
								<kendo:grid-column title="Closing Adv" field="closing_balance"
									filterable="true" width="95px" aggregates="sum"
									groupFooterTemplate="#: kendo.toString(sum, 'n2') #"
									footerTemplate="#:kendo.toString(closing_balance, 'n2','0.00') #" />
									...
									<kendo:dataSource-schema-model id="id">
										<kendo:dataSource-schema-model-fields>
											...
											<kendo:dataSource-schema-model-field name="wardNo"
												type="string" />
											<kendo:dataSource-schema-model-field name="denoted_by"
												type="string" />
											<kendo:dataSource-schema-model-field name="opening_adv"
												type="number" />
											...
											<kendo:dataSource-schema-model-field name="total_bill"
												type="number" />
											<kendo:dataSource-schema-model-field name="adjustments"
												type="number" />
											<kendo:dataSource-schema-model-field name="current_adv_amt"
												type="number" />
											<kendo:dataSource-schema-model-field name="current_adv_rebate"
												type="number" />
											<kendo:dataSource-schema-model-field name="closing_balance"
												type="number" />
...
<script>
	...
	function dataBound(e) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			adjustments = adjustments + data[i].adjustments;
			current_adv_amt = parseFloat(current_adv_amt) + parseFloat(data[i].current_adv_amt);
			current_adv_rebate = current_adv_rebate + data[i].current_adv_rebate;
		...
		current_adv_amt =parseFloat(current_adv_amt).toFixed(2);
		current_adv_rebate =parseFloat(current_adv_rebate).toFixed(2);
	...
	function printGrid() {
		...
		var htmlStart = '<!DOCTYPE html>'
				...
				+ '<head>'
				+ '<meta charset="utf-8" />'
				+ '<title>Advance Collection Report</title>'
				...
				+ '<h2>' + branch
				+ ' kathmandu</h2><h3>Advance Collection Report</h3>'
	...
	function getData() {
		...
		if (yearnep == "") {
			...
			$
					.ajax({
						type : "GET",
						url : "./advanceCollectionReport/advanceCollectionReportReadNew",
						dataType : "JSON",
						async : false,
	...
	function generateAdvanceReport(){
		...
		if(username!='s_admin'){
			alert('You Are not authorised to Generate Report');
			return false;
		}
		
		var yearnep = $("#yearnep").val();
		var month = $("#month").val();
		var fromdt = $("#fromDatenep").val();
		var todt = $("#toDatenep").val();
		...
		if (yearnep == "") {
			...
			$
					.ajax({
						type : "GET",
						url : "./advanceCollectionReportGenerate",
						dataType : "text",
						async : false,
