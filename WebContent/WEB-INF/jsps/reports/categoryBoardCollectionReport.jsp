	...
	<div class="panel panel-flat">
		<div class="panel-body">
			<fieldset class="content-group">
				<legend class="text-bold">Category Board Collection Report</legend>
				<div class="row">
				...
				<div class="text-center">
					...
					<button type="button" class="btn bg-teal btn-ladda"
						...
						<span class="ladda-label">Get Collection Data</span>
					</button>
				</div>
		...
		<%--  <kendo:grid-excel allPages="true" fileName="Kendo UI Grid Export.xlsx" filterable="true"  /> footerTemplate="<span id='sumTotal_Earnings'></span>"--%>
			<kendo:grid-editable mode="popup" confirmation="Are you sure you want to remove this item?" />
				<kendo:grid-columns>
					...
					<kendo:grid-column title="Category" field="category" width="120px" filterable="true"  groupFooterTemplate="Total :" footerTemplate="Grand&nbsp;Tot:" />
					<kendo:grid-column title="Con Type" field="contype" width="80px" filterable="true" />
					<kendo:grid-column title="Opening Bal" field="openingBal" width="100px" filterable="true" aggregates="sum" groupFooterTemplate="#: kendo.toString(sum, 'n2') #" footerTemplate="#:kendo.toString(openingBal,'n2') #" />
					<kendo:grid-column title="Arrears" field="arrears" width="100px" filterable="true" aggregates="sum" groupFooterTemplate="#: kendo.toString(sum, 'n2') #" footerTemplate="#:kendo.toString(arrears,'n2') #" />
					<kendo:grid-column title="Penalty" field="penalty" width="100px" filterable="true" aggregates="sum" groupFooterTemplate="#: kendo.toString(sum, 'n2') #" footerTemplate="#:kendo.toString(penalty,'n2') #" />
					<kendo:grid-column title="Penalty Adj" field="penaltyAdj" width="100px" filterable="true" aggregates="sum" groupFooterTemplate="#: kendo.toString(sum, 'n2') #" footerTemplate="#:kendo.toString(penaltyAdj,'n2') #" />
					<kendo:grid-column title="Paid" field="totalPaid" width="95px" filterable="true" aggregates="sum" groupFooterTemplate="#: kendo.toString(sum, 'n2') #" footerTemplate="#:kendo.toString(totalPaid,'n2') #" />
					<kendo:grid-column title="Close Bal" field="closingBal" width="95px" filterable="true" aggregates="sum" groupFooterTemplate="#: kendo.toString(sum, 'n2') #" footerTemplate="#:kendo.toString(closingBal,'n2') #" />
								...
								<kendo:dataSource-schema>
									<kendo:dataSource-schema-model id="id">
										<kendo:dataSource-schema-model-fields>
											...
											<kendo:dataSource-schema-model-field name="contype" type="string" />
											<kendo:dataSource-schema-model-field name="openingBal" type="number" />
											<kendo:dataSource-schema-model-field name="arrears" type="number" />
											<kendo:dataSource-schema-model-field name="penalty" type="number" />
											<kendo:dataSource-schema-model-field name="totalPaid" type="number" />
											<kendo:dataSource-schema-model-field name="closingBal" type="number" />
											<kendo:dataSource-schema-model-field name="penaltyAdj" type="number" />
										</kendo:dataSource-schema-model-fields>
...
<!--  ward_no,opening_arrears,water_charge,sw_charge,meter_rent,penalty,rebate,adv_payment,next_payment,arrear_balance -->
	...
	function printGrid() {
		...
		var htmlStart = '<!DOCTYPE html>'
				...
				+ '<head>'
				+ '<meta charset="utf-8" />'
				+ '<title>Category Board Collection Report</title>'
				...
				+ '<h2>'+branch+' kathmandu</h2><h3>Category Board Collection Report</h3>'
				+ '<h4>From :&nbsp;&nbsp;' + frm
				+ '&nbsp;&nbsp;&nbsp;To :&nbsp;&nbsp;' + to + '&nbsp;&nbsp;</h4></center>';
	...
	function getDailyRevenueData() {
		...
		/* if (category == "" || category ==null) {
			...
			$.ajax({
				type : "GET",
				url : "./categoryBoardCollectionReport/categoryBoardCollectionReportRead",
				dataType : "JSON",
				async : false,
				data : {
					from_date : from_date,
...
function clearButton(){
		...
		window.location.href="./categoryBoardCollectionReport";
	}
