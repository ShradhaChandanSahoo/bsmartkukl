...
<c:url value="./counterRevenueReport" var="readCategoryUrl" />
	...
	<div class="panel panel-flat">
		<div class="panel-body">
			<fieldset class="content-group">
				<legend class="text-bold">Counter Wise Revenue Report</legend>
				<div class="row">
		...
		<%--  <kendo:grid-excel allPages="true" fileName="Kendo UI Grid Export.xlsx" filterable="true"  /> footerTemplate="<span id='sumTotal_Earnings'></span>"--%>
			<kendo:grid-editable mode="popup" confirmation="Are you sure you want to remove this item?" />
				<kendo:grid-columns>
							...
							<kendo:grid-column title="CounterNo" field="counterno" width="95px" filterable="true" />
								...
								<kendo:dataSource-schema>
									<kendo:dataSource-schema-model id="id">
										<kendo:dataSource-schema-model-fields>
											...
											<kendo:dataSource-schema-model-field name="counterno" type="number" />
										</kendo:dataSource-schema-model-fields>
...
<!--  ward_no,opening_arrears,water_charge,sw_charge,meter_rent,penalty,rebate,adv_payment,next_payment,arrear_balance -->
	...
	function printGrid() {
		...
		var htmlStart = '<!DOCTYPE html>'
				...
				+ '<head>'
				+ '<meta charset="utf-8" />'
				+ '<title>Counter Wise Revenue Report</title>'
				...
				+ '<h2>'+branch+' kathmandu</h2><h3>Counter Wise Revenue Report</h3>'
	...
	function getDailyRevenueData() {
		...
		if (startDate > endDate) {
			...
			$.ajax({
				type : "GET",
				url : "./counterRevenueReport/counterRevenueReportRead",
				dataType : "JSON",
				async : false,
				data : {
					from_date : from_date,
...
function clearButton(){
		...
		window.location.href="./counterRevenueReport";
	}
