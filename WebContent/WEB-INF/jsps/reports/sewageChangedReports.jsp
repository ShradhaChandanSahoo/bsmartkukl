	...
	<div class="panel panel-flat">
		<div class="panel-body">
			<fieldset class="content-group">
				<legend class="text-bold">Sewage Changed Report</legend>
				<div class="row">
					...
					<div class="form-group col-md-4">
					 ...
					 <div class="form-group col-md-4">
						<label>Sewage Changed To <font color="red">*</font></label> 
						<select class="select" id="category" name="category" required="required" data-placeholder="Select Category">
						<option value="" data-icon="icon-git-branch">Sewage Changed To</option>
							<option value="1" data-icon="icon-git-branch">Yes</option>
							<option value="2" data-icon="icon-git-commit">No</option>
							<option value="3" data-icon="icon-git-commit">All</option>
				...
				<div class="text-center">
					...
					<label>&nbsp;</label>
					<button type="button" class="btn bg-teal btn-ladda"
						onclick="return getNewConnDetails()" data-style="expand-right"
						data-spinner-size="20">
						<span class="ladda-label">Get Sewage Changed Data</span>
					</button>
				</div>
			...
			<div class="wethear" style="overflow: scroll;">
				<fieldset class="content-group">
					<legend class="text-bold">Sewage Changed Report Details</legend>
		...
		<%--  <kendo:grid-excel allPages="true" fileName="Kendo UI Grid Export.xlsx" filterable="true"  /> footerTemplate="<span id='sumTotal_Earnings'></span>"--%>
			<kendo:grid-editable mode="popup" confirmation="Are you sure you want to remove this item?" />
				<kendo:grid-columns>
					...
					<kendo:grid-column title="Connection No" field="connNo" width="80px" filterable="true"   />
					<kendo:grid-column title="Month Year" field="monthyr" width="100px" filterable="true"  />
					<kendo:grid-column title="Old SC" field="scOld" width="100px" filterable="true"  />
					<kendo:grid-column title="New SC" field="scNew" width="100px" filterable="true"  />
					<kendo:grid-column title="Approved By" field="approvedBy" width="100px" filterable="true" />
					<kendo:grid-column title="Approved Date" field="approvedDate" width="100px" filterable="true"  />
					<kendo:grid-column title="Remarks" field="remarks" width="100px" filterable="true"  />
								...
								<kendo:dataSource-schema>
									<kendo:dataSource-schema-model id="id">
										<kendo:dataSource-schema-model-fields>
											...
											<kendo:dataSource-schema-model-field name="monthyr" type="string" />
											<kendo:dataSource-schema-model-field name="scOld" type="string" />
											<kendo:dataSource-schema-model-field name="scNew" type="string" />
											<kendo:dataSource-schema-model-field name="approvedBy" type="number" />
											<kendo:dataSource-schema-model-field name="approvedDate" type="number" />
 ...
 function onChange(arg) {
	...
	function printGrid() {
		...
		var htmlStart = '<!DOCTYPE html>'
				...
				+ '<h2>'+branch+' kathmandu</h2><h3>New Connection Report</h3>'
				+ '<h4>From :&nbsp;&nbsp;' + frm
				+ '&nbsp;&nbsp;&nbsp;To :&nbsp;&nbsp;' + to + '&nbsp;&nbsp;</h4></center>';
	...
	function getNewConnDetails() {
		...
		if (startDate > endDate) {
		 ...
		 if (category == "" || category ==null) {
			alert("Please Select New Connection");
			return false;
		} 
		else {
			$('#loading').show();
			$.ajax({
				type : "GET",
				url : "./sewageChangedReportDetails", ///newConnectionReportDetails
				dataType : "JSON",
				async : false,
				data : {
					from_date : from_date,
...
function clearButton(){
		...
		window.location.href="./newConnectionReport";
	}
