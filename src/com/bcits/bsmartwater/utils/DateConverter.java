...
import static com.bcits.bsmartwater.utils.Lookup.lookup;
import static com.bcits.bsmartwater.utils.Lookup.lookupNepaliYearStart;
import static com.bcits.bsmartwater.utils.Lookup.monthDays;
   ...
   // static Logger //logger = LoggerFactory.getLogger(DateConverter.class);
	...
	if (lookupNepaliYearStart <= bsYear
		...
		if (bsDayOfMonth <= dayOfMonth) {
		    ...
		    //logger.warn("invalid day of month " + bsDayOfMonth			    + " for year " + bsYear + " and month " + bsMonth);
	...
	if (format.equals("ddMMyyyy")) {
	    //logger.debug("date format wants to test is " + format		    + " real text is " + bsDate);
